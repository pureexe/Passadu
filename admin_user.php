...
$sql = "SELECT * FROM user JOIN major ON `user`.`major_id` = `major`.`major_id`";//คำสั่งในการเลือกข้อมูล
...
<?php }?>
    ...
    <div class="form-group">
        <label class="control-label col-md-2" for="u-user_type">สาขา</label>
        <div class="col-md-10">
          <select name="u[major_id]" class="form-control" id="u-marjor">
            <?php
              $sql = "SELECT * FROM major ORDER BY major_id ASC";//คำสั่งในการเลือกข้อมูล
              $resultz = $con->prepare($sql);//เตรียมคำสั่ง SQL
              $resultz->execute();//ประมวลผล
              while($rs=$resultz->fetch()){
            ?>
				        <option value="<?php echo $rs["major_id"]; ?>"><?php echo $rs[major]; ?></option>
            <?php
              }
            ?>
			   </select>
    ...
    <div class="form-group">
        <label class="control-label col-md-2" for="u-user_type">กลุ่มผู้ใช้งาน</label>
        <div class="col-md-10">
